...
export async function checkKnowledge(
  ...
  // デバッグログ: 動的プロフィールの内容を確認（開発環境のみ詳細表示）
  if (process.env.NODE_ENV === 'development') {
    console.log('[知識判定] 動的プロフィール件数:', dynamicProfile.length);
    console.log('[知識判定] 動的プロフィール内容:', dynamicProfileText);
  } else {
    console.log('[知識判定] 動的プロフィール:', dynamicProfile.length, '件');
  }
...
1. **回答可能（canAnswer: true）の場合**
   - 上記プロフィール情報に直接関連する質問
   - 動的プロフィールに関連する情報がある場合（質問の品数や具体的な表現が異なっても、関連情報があれば回答可能）
   - 技術的な一般知識（プログラミング全般など）
   - 挨拶・雑談（こんにちは、元気？など）
   - サイト生成のリクエスト
   - 記事に関する質問
   - **誘い・提案・約束系の質問**
     例: 「一緒に〇〇に行きませんか？」「今度〇〇しましょう」「会いたいです」
     → これらは肯定して問い合わせに誘導すべきなので、回答可能

2. **回答不可（canAnswer: false）の場合**
   - プロフィールにない具体的な個人情報の質問
     例: 「好きな食べ物は？」「好きな映画は？」（プロフィールに記載がない場合）
   - その話題に関する情報が動的プロフィールに一切ない場合
     例: 「好きなラーメン屋を教えて」（ラーメンに関する情報がプロフィールにない）

3. **重要: 品数・個数に関係なく関連情報があれば回答可能**
   - プロフィールに「ガストのメニュー」についての情報があれば、「1個教えて」でも「100個教えて」でも回答可能
   - 持っている情報の範囲内で最大限答えればよい
   - 例: ガストについての情報があれば→「ガストの好きなメニューを3個教えて」→ canAnswer: true
   - 例: ガストについての情報がなければ→「ガストの好きなメニューを1個教えて」→ canAnswer: false

4. **重要: 誘い・提案は必ず回答可能**
   - 「〇〇に行きませんか？」「一緒に〇〇しよう」「会えますか？」などの誘いは必ず canAnswer: true
   - プロフィールに情報がなくても、誘いには肯定的に返答してお問い合わせに誘導できる
   - shouldRecord: false（学習する必要なし）

5. **判断基準**
   - プロフィールに関連する話題があれば回答可能（質問の詳細な条件は柔軟に対応）
